...
struct TourOverlayView: View {
        ...
        return AnyView(
            GeometryReader { geo in
                ZStack(alignment: .bottom) {
                    // ── Backdrop (content only, tab bar exposed) ──
            ...
            .id(tutorial.currentStep) // re-animate card on step change
        )
    }
    ...
    private func tabArrowLeading(geo: GeometryProxy, tab: Int) -> CGFloat {
        ...
        // Subtract half of arrow width (~6pt) + card horizontal padding (10pt)
        return max(10, center - 6)
    }

    private func tabBarHeight(geo: GeometryProxy) -> CGFloat {
        // Tab bar is ~49pt + bottom safe area inset
